...
const char test_vp[] = "!!ARBvp1.0\n"
                       ...
                       "MOV result.fogcoord, 0.2;\n"
                       "END";

const char test_fp[] = "!!ARBfp1.0\n"
                       "OPTION NV_fragment_program2; # want conditional execution - doesn't affect reported issue\n"
                       "# fogcoord is defined as having components (f,0,0,1)...\n"
                       "TEMP fc; MOV fc, fragment.fogcoord; # full four-component vector\n"
                       "TEMP fc_x; MOV fc_x, fragment.fogcoord.x; # should equal 0.2 (set by vertex program)\n"
                       "TEMP fc_y; MOV fc_y, fragment.fogcoord.y; # should equal 0\n"
                       "TEMP fc_z; MOV fc_z, fragment.fogcoord.z; # should equal 0\n"
                       "TEMP fc_w; MOV fc_w, fragment.fogcoord.w; # should equal 1\n"
                       "\n"
                       "TEMP col;\n"
                       "TEX col, fragment.texcoord[0], texture[0], 2D;\n"
                       "\n"
                       "TEMP cmp;\n"
                       "SUBC cmp, fragment.texcoord[0].x, 0.7;\n"
                       "IF GE.x;\n"
                       "    # mess with output colour so rendering is wrong if directly accessed/swizzled components are incorrect\n"
                       "    # (will result in no change if everything is correct)\n"
                       "    ADD col, col, fc_x; SUB col, col, 0.2;\n"
                       "    ADD col, col, fc_y; SUB col, col, 0;\n"
                       "    ADD col, col, fc_z; SUB col, col, 0;\n"
                       "    ADD col, col, fc_w; SUB col, col, 1;\n"
                       "ELSE;\n"
                       "    # show that the full four component vector fc was fine, only swizzled access is incorrect\n"
                       "    ADD col, col, fc.x; SUB col, col, 0.2;\n"
                       "    ADD col, col, fc.y; SUB col, col, 0;\n"
                       "    ADD col, col, fc.z; SUB col, col, 0;\n"
                       "    ADD col, col, fc.w; SUB col, col, 1;\n"
                       "ENDIF;\n"
                       "\n"
